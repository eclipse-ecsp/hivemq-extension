 ...
 */
public class SimulatorTopicFormatter extends AbstractTopicMapper {
    private static final IgniteLogger LOGGER = IgniteLoggerFactory.getLogger(SimulatorTopicFormatter.class);
    ...
    /**
     * Retrieves the topic mapping based on the supplied MQTT topic.
     *
     * @param mqttTopic The MQTT topic to be mapped.
     * @return The TopicMapping object containing the mapped topic information.
     * @throws IllegalTopicArgumentException If the MQTT topic is not proper.
    ...
    public TopicMapping getTopicMapping(String mqttTopic) throws IllegalTopicArgumentException {
        ...
        String[] topicToken = mqttTopic.split(MQTT_DELIMITER, TOKEN_LENGTH);
        if (topicToken.length < TOKEN_LENGTH) {
    ...
    /**
     * Initializes the SimulatorTopicFormatter with the provided properties.
     *
    ...
    /**
     * Returns the connect topic for the simulator.
     *
     * @return The connect topic.
     */
    @Override
    ...
    /**
     * Returns the topic used for disconnect messages.
     *
