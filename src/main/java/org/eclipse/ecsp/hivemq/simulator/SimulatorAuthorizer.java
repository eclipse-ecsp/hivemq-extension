...
public class SimulatorAuthorizer extends SimulatorAbstractAuthorization {
    private static final IgniteLogger LOGGER = IgniteLoggerFactory.getLogger(SimulatorAuthorizer.class);
     ...
     */
    public SimulatorAuthorizer() throws EmptyOrNotAvailablePropertyException {
        try {
     ...
     * @return A list of topic permissions granted to the client.
     */
    @Override
    public List<TopicPermission> authorize(InitializerInput initializerInput) {
        ...
        if (userName.isEmpty()) {
            LOGGER.warn("Invalid clientdata : {}", initializerInput.getClientInformation());
        ...
        LOGGER.info("mqttUserName: {}, mqttUserPrefix: {}  user: {}", userName, mqttUserPrefixList, user);
        if (HivemqUtils.isWhiteListedUser(user)) {
            mqttTopicPermissions = getWhiteUserPermission(userName);
            ...
            // for device
            mqttTopicPermissions = getDevicePermission(userName, clientId);
        }
     ...
     * @return List of mqtt topic permission 
     */
    protected List<TopicPermission> getDevicePermission(String userName, String clientId) {
        ...
        if (mqttTopicPermissions != null) {
            LOGGER.debug("Returning permissions from cache for clientId: {}", clientId);
            return mqttTopicPermissions;
        }
    ...
    /**
     * Prepares a list of topic permissions for a device based on the provided configuration.
     *
     ...
     * @return                              The list of topic permissions for the device.
     */
     ...
     *
     * @param mqttTopicPermissions the list of MQTT topic permissions to add the custom permissions to
     */
    private void addCustomTopicPermissions(List<TopicPermission> mqttTopicPermissions) {
        ...
        if (extraTopics != null && !extraTopics.isEmpty()) {
            mqttTopicPermissions.addAll(extraTopics);
        }
    }

    /**
     * Retrieves the white user permissions for the specified user.
     *
     * @param userName the name of the user
     * @return a list of TopicPermission objects representing the white user permissions
     */
    protected List<TopicPermission> getWhiteUserPermission(String userName) {
        ...
        LOGGER.info("Whitelisted user: {} , got all permission to topic {}", userName, topicName);
        return mqttTopicPermissions;
    }
    ...
    public List<TopicPermission> getSubscribeTopicPermissions(String clientId, String userName,
        ...
        LOGGER.info("clientId: {} got subscribe permission to topics {}", clientId, topicPermissions);

        return mqttTopicPermissions;
    }

    /**
     * Adds global topic permissions to the given list of MQTT topic permissions.
     *
     * @param mqttTopicPermissions the list of MQTT topic permissions to add global topic permissions to
     */
    ...
    public List<TopicPermission> getPublishTopicPermissions(String clientId, String userName, List<String> mqttTopics) {
        ...
        mqttTopicPermissions.forEach(mqttTopicPermission -> topicPermissions
                .append(mqttTopicPermission.getTopicFilter()).append(StringUtils.SPACE));
        LOGGER.info("clientId: {} got publish permission to topics {}", clientId, topicPermissions);

        return mqttTopicPermissions;
    }

    /**
     * Sets the topic formatter instance for the simulator authorizer.
     *
